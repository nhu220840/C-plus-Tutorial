...
int main(){
    ...
    cin.ignore();
    string searchAddress; getline(cin, searchAddress);
    cout << "DANH SACH GIAO VIEN CO DIA CHI TAI " << searchAddress << " :" << endl;
    for(Lecturer x : listOfLecturers){
        if(x.Person::getAddress() == searchAddress)
            x.print();
    }
    cout << "DANH SACH SINH VIEN CO DIA CHI TAI " << searchAddress << " :" << endl;
